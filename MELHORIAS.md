...
- [x] ~~**`try/catch` engolindo erro nas duas páginas públicas**~~ — resolvido em 10/08/2026 (ver `## ✅ Resolvido`). O padrão continua aberto no resto do app: o item genérico é o "**`error` ignorado em quase toda query**" acima.
...
1. **Aplicar a migration 00014** — o catálogo público (`/cursos` e a home) está fora do ar para visitante deslogado desde que a 00012 rodou. É uma linha de `grant`. As 00001→00012 já estão aplicadas e verificadas.
...
- [x] **Catálogo público fora do ar para quem não estava logado — regressão introduzida por mim na 00012** — `supabase/migrations/00014_fix_get_my_role_grant.sql`. A 00012 terminava com `revoke execute on function public.get_my_role() from public, anon`, justificando que a função "é usada dentro das policies, nunca pela aplicação, e as policies a chamam no contexto do servidor". **A justificativa está errada:** expressão de RLS é avaliada como o *papel que consulta*, não como o dono da tabela — se a policy chama uma função, o chamador precisa de `EXECUTE`. `courses` tem três policies permissivas de SELECT e duas delas (`courses_admin_all`, `courses_teacher_own_all`) chamam `get_my_role()`; o Postgres avalia as permissivas em conjunto, então o visitante anônimo esbarrava nelas mesmo querendo só a `courses_public_approved_read`. Reproduzido com a chave `anon`: `HTTP 401 {"code":"42501","message":"permission denied for function get_my_role"}`. Passou despercebido por dois motivos que se somaram: `authenticated` manteve o grant (todo teste com sessão aberta funcionava) e `/cursos` engolia o erro, mostrando "O catálogo está sendo montado" — que parece estado normal de produto novo. As outras revogações da 00012 continuam corretas e não devem ser desfeitas: `create_product_order` e as `get_admin_*` são chamadas por `.rpc()` da aplicação, não por policy nenhuma. **Regra que fica:** antes de revogar `EXECUTE`, rodar o `select ... from pg_policies where qual::text like '%nome_da_funcao%'` que está no cabeçalho da 00014.
- [x] **As duas páginas públicas disfarçavam falha de permissão de catálogo vazio** — `app/(public)/cursos/page.tsx`, `app/(public)/page.tsx`. É a outra metade do item acima: `supabase-js` não lança em erro HTTP, devolve `{ data: null, error }`, então o `try/catch` que já existia nunca via o 401 — a lista vinha vazia e a tela renderizava o convite "Se você cozinha para viver, esse espaço pode ser seu". As duas passaram a fazer `if (error) throw error` e `console.error`, e `/cursos` ganhou um terceiro estado visual (vermelho, "Não foi possível carregar o catálogo", com "Tentar de novo") separado dos dois vazios legítimos — sem filtro e com filtro. Falha de infraestrutura agora aparece como falha.
...
- [x] **Formulário travado para sempre quando o Supabase não está configurado** — `components/auth/LoginForm.tsx`, `components/auth/RegisterForm.tsx`, `components/curso/PurchaseBox.tsx`. Flagrado em produção: o `createClient()` corrigido acima lança com mensagem clara, mas era chamado **depois** do `setLoading(true)` e **fora** de qualquer `try`. A exceção subia sem tratamento, o `setLoading(false)` nunca rodava e o botão ficava preso em "Criando conta…" / "Entrando…" indefinidamente — sem erro na tela e sem como tentar de novo. No `PurchaseBox` o efeito era o mesmo com outra cara: status preso em `'checking'`, um retângulo pulsando eterno no lugar da caixa de compra. Os três passaram a tratar a exceção e mostrar a mensagem. São exatamente os três componentes client alcançáveis nesse cenário — sem Supabase ninguém loga, então os de dashboard nunca chegam a montar.
